...
from ftdi_async_interface import *
...
class BusInterface:
    ...
    def __init__(self, iface_type = 'ftdi', iface = None):
        if iface_type == "ftdi_async":
            self.bus = FtdiAsyncInterface(iface)
        elif iface_type == "ftdi":
            self.bus = FtdiSyncInterface(iface)            
        else:
            raise ValueError('Invalid interface type specified !(ftdi_async|ftdi).')
